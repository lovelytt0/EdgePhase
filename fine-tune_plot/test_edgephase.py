...
#     folder = os.path.join(save_figs, 'event_'+str(idx),'eqt')

#     if os.path.isdir(folder):
#         shutil.rmtree(folder)  
#     os.makedirs(folder)   
#     X = test_generator.__getitem__(idx)
#     y_pre =  eqt_model.forward(X[0])
#     for ts in range(len(X[0])): 

#         _plotter(X[0][ts], X[1][ts], y_pre[1][ts,:].detach().numpy(), y_pre[2][ts,:].detach().numpy(), folder, str(ts))
...
#     plt.title('Fine-tuned Eqtransformer with EdgeConv')
...
#     plt.savefig('Example/'+str(idx)+'_edge.png')
...
# dict['miss_p']=miss_p
# dict['miss_s']=miss_s
# dict['match_p']=match_p
# dict['match_s']=match_s
# dict['false_p']=false_p
# dict['false_s']=false_s
# dict['delta_p']=delta_p
# dict['delta_s']=delta_s
# dict['add_p']=add_p
# dict['add_s']=add_s
# out_file_name= os.path.join(folder2,'th_'+str(threshold)+'_res.pkl')
