...
#     plt.title('Eqtransformer without fine-tuning')
...
#         s=_detect_peaks(S_seq, mph=0.06, mpd=1)
...
#     plt.savefig('Example/'+str(idx)+'_no_finetune.png')
...
#     y_pre =  torch.sigmoid(eqt_model.forward(X[0])).detach().numpy()

    for sta in range(len(P_pred)):
        P_seq, S_seq = P_pred[sta], S_pred[sta]
        ...
        res_p.append((np.array(p)/100.0,np.array(P_seq[p]),target_p,snr))
        res_s.append((np.array(s)/100.0,np.array(S_seq[s]),target_s,snr))
