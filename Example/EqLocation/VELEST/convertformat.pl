...
foreach $file(@par){
    ...
    if($test eq "#"){
        ...
        if($count % 300 == 1){
        $folder++;
        print "start new folder\n";
        if($folder > 1){
        close(EV);
        close(CT);
        }
        $phaseout = "greece$folder.pha"; # phase format for VELEST ISED=1
        $phasecat = "greece$folder.cat"; # catalog format for VELEST
